...
--*/
{
    ...
    m_ullSideDockMicReadPosition = 0;
    m_ullDmaTimeStamp = 0;
    ...
    if (m_bCapture)
    {
        (void)SideDockMicBridgeEnsureMapped();
    ...
    {
        (void)SideDockSpeakerBridgeEnsureMapped();
    }
...
)
{
    ...
    else
    {
        ...
        // Read render data from the WaveRT buffer and publish it for the host.
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        SideDockMicBridgeRead(&m_ullSideDockMicReadPosition, m_pDmaBuffer + bufferOffset, runWrite);
...
--*/
{
    ...
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
        SideDockSpeakerBridgeWrite(m_pDmaBuffer + bufferOffset, runWrite);
